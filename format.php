...
class qformat_giftmedia extends qformat_gift {
    ...
    // Unfortunately we need to duplicate some functions of the GIFT format just to remove the
    // infamous files element.
    protected function commentparser($answer, $defaultformat) {
        ...
        if (count($bits) > 1) {
            ...
            $feedback = array('text' => '', 'format' => $defaultformat);
        }
    ...
    protected function split_truefalse_comment($answer, $defaultformat) {
        ...
        if (count($bits) > 1) {
            ...
            $wrongfeedback = array('text' => '', 'format' => $defaultformat);
        }
        if (count($bits) > 2) {
            $rightfeedback = $this->parse_text_with_format(trim($bits[2]), $defaultformat);
        } else {
            $rightfeedback = array('text' => '', 'format' => $defaultformat);
        }
